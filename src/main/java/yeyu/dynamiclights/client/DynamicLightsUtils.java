...
public class DynamicLightsUtils {
    ...
    public static boolean handleEntityUnlit(BlockPos bp, int entityId, ClientWorld clientWorld) {
        ...
        if (maxLight > 0) {
            DynamicLightsOptions.getLightsLevel().iterateLightMap(cameraPosVec, clientWorld, maxLight);
            return true;
        }
        return false;
    ...
    public static void handleEntity(Entity entity, ClientWorld clientWorld, int fixedLightLevel, Integer lightEnchantmentInt, Integer lightFireInt, Integer lightExplosionIgnitionInt, boolean animate) {
        ...
        if (!animate) {
            ...
            DynamicLightsOptions.getLightsLevel().iterateLightMap(cameraPosVec, clientWorld, 0, true);
            return;
        }
    ...
    public static void handleEntityLightsOnPickup(Entity entity, ItemEntity itemPicked, ClientWorld clientWorld) {
        ...
        float maxLight = DynamicLightsStorage.animationFactor(entity, itemLightLevel);
        if (maxLight > 0)
            DynamicLightsOptions.getLightsLevel().iterateLightMap(cameraPosVec, clientWorld, maxLight, true);
    }
